<div class="toggle-container filters-container">
        ...
        <div id="center-categories-filter" class="panel-collapse collapse in">
            <div class="panel-content">
                <ul class="filters-option">
                    ...
                    <li><a href="{{ route('courses') }}">@lang('messages.frontend-courses')<small>({{$courseCount}})</small></a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="panel style1 arrow-right">
        <h4 class="panel-title">
            <a data-toggle="collapse" href="#course-categories-filter" class="">@lang('messages.frontend-course-categories')</a>
        </h4>
        <div id="course-categories-filter" class="panel-collapse collapse in">
            <div class="panel-content">
                <ul class="filters-option">
                    <li @if($type == 'courses' && $categorySlug == null) class="active" @endif><a href="{{ route('courses') }}">@lang('messages.frontend-all')<small>({{$courseCount}})</small></a></li>
                    @foreach($courseCategories as $key => $category)
                        <li @if($type == 'courses' && $categorySlug == $category->slug ) class="active" @endif><a href="{{ route('courses') }}?danh-muc={{ $category->slug }}">{{ $category->name }}<small>({{ count($category->courses) }})</small></a></li>
                    @endforeach
                </ul>
            </div>
        </div>
    ...
    <div class="panel style1 arrow-right">
        ...
        <div id="doc-categories-filter" class="panel-collapse collapse in">
            <div class="panel-content">
                <ul class="filters-option">
                    <li @if($categorySlug == null && $type == 'documents') class="active" @endif><a href="{{ route('documents') }}">@lang('messages.frontend-all')<small>({{$documentCount}})</small></a></li>
                    @foreach($docCategories as $key => $category)
                        <li @if($categorySlug == $category->slug && $type == 'documents') class="active" @endif><a href="{{ route('documents') }}?danh-muc={{ $category->slug }}">{{ $category->name }}<small>({{ count($category->documents) }})</small></a></li>
                    @endforeach
                </ul>
            </div>
        </div>
