<div class="jquery-accordion-menu white">
    <a href="{{ route('tests') }}"><div class="jquery-accordion-menu-header uppercase"><i
                class="fa fa-th"></i> @lang('messages.frontend-test-categories') </div></a>
    <ul>
        @php
            $parent_id = 0;
            $last_name = '';
    ...
    @if (count($category->children()) > 0)
        <li @if($categorySlug == $category->slug && $type == 'tests') class="active" @endif>
            <a href="{{ route('tests') }}?danh-muc={{ $category->slug }}"><i
                        class="fa fa-angle-double-right"></i> {{ $category->name }} ({{ count($category->test) }})</a><span class="submenu-indicator"><i class="fa fa-plus"></i></span>
            <ul class="submenu" @if(in_array($category->slug,$parentArrayTest)) style="display: block" @endif>
                ...
                @if (count($category->children()) == 0)
                    <li @if($categorySlug == $category->slug && $type == 'tests') class="active" @endif>
                        <a href="{{ route('tests') }}?danh-muc={{ $category->slug }}"><i
                                    class="fa fa-angle-double-right"></i>{{ strlen($category->name)>(30+$level*24)? substr($category->name, 0, strpos($category->name,' ',25+$level*24)).'...' : $category->name }} ({{ count($category->test) }})</a>
                    </li>
                    @php
...
<div class="jquery-accordion-menu white">
    <a href="{{ route('documents') }}"><div class="jquery-accordion-menu-header uppercase"><i
                class="fa fa-th"></i> @lang('messages.frontend-doc-categories') </div></a>
    <ul>
        @php
            $parent_id = 0;
            $last_name = '';
...
<div class="jquery-accordion-menu white">
    <a href="{{ route('centers') }}"><div class="jquery-accordion-menu-header uppercase"><i
                class="fa fa-th"></i> @lang('messages.frontend-center-categories') </div></a>
    <ul>
        <li @if($type == 'centers') class="active" @endif><a href="{{ route('centers') }}"><i
                        class="fa fa-angle-double-right"></i> @lang('messages.frontend-centers') ({{ $centerCount - 1 }})</a></li>
        <li @if($type == 'teachers') class="active" @endif><a href="{{ route('teachers') }}"><i
                        class="fa fa-angle-double-right"></i> @lang('messages.frontend-teachers') ({{ $teacherCount - 1 }})</a></li>
        <li @if($type == 'courses') class="active" @endif><a href="{{ route('courses') }}"><i
                        class="fa fa-angle-double-right"></i> @lang('messages.frontend-courses') ({{ $courseCount }})</a></li>
    </ul>
</div>
