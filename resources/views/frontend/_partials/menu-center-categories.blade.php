...
<div class="jquery-accordion-menu white">
    ...
    @if (count($category->children()) > 0)
        <li @if($categorySlug == $category->slug && $type == 'documents') class="active" @endif>
            <a href="{{ route('documents') }}?danh-muc={{ $category->slug }}">
                <i class="fa fa-angle-double-right"></i> {{ strlen($category->name)>25+$level*24? substr($category->name, 0, strpos($category->name,' ',25+$level*24)).'...' : $category->name }} ({{ count($category->documents) }})
            </a>
            ...
            <ul class="submenu">
                ...
                @if (count($category->children()) == 0)
                    <li @if($categorySlug == $category->slug && $type == 'documents') class="active" @endif>
                        <a href="{{ route('documents') }}?danh-muc={{ $category->slug }}">
                            <i class="fa fa-angle-double-right"></i>{{ strlen($category->name)>30+$level*24? substr($category->name, 0, strpos($category->name,' ',30+$level*24)).'...' : $category->name }} ({{ count($category->documents) }})
                        </a>
                    </li>
                    @php
...
<div class="jquery-accordion-menu white">
    <a href="{{ route('tests') }}"><div class="jquery-accordion-menu-header uppercase"><i class="fa fa-th"></i> @lang('messages.frontend-test-categories') </div></a>
    <ul>
        @php
        $parent_id = 0;
        $last_name = '';
    ...
    @if (count($category->children()) > 0)
        <li @if($categorySlug == $category->slug && $type == 'tests') class="active" @endif>
            <a href="{{ route('tests') }}?danh-muc={{ $category->slug }}">
                <i class="fa fa-angle-double-right"></i> {{ strlen($category->name)>25+$level*24? substr($category->name, 0, strpos($category->name,' ',25+$level*24)).'...' : $category->name }} ({{ count($category->test) }})
            </a>
            <span class="submenu-indicator"><i class="fa fa-plus"></i></span>
            <ul class="submenu">
                ...
                @if (count($category->children()) == 0)
                    <li @if($categorySlug == $category->slug && $type == 'tests') class="active" @endif>
                        <a href="{{ route('tests') }}?danh-muc={{ $category->slug }}">
                            <i class="fa fa-angle-double-right"></i>{{ strlen($category->name)>30+$level*24? substr($category->name, 0, strpos($category->name,' ',30+$level*24)).'...' : $category->name }} ({{ count($category->test) }})
                        </a>
                    </li>
                    @php
                    $parent_id = $category->parent_id;
