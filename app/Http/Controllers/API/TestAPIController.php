...
class TestAPIController extends AppBaseController
{
    ...
    public function __construct(TestRepository $testRepo, CategoryTestRepository $categoryTestRepo)
    {
     ...
     *
     * @SWG\Get(
     *      path="/tests/{id}/comments",
     *      summary="get all comments of Test",
     *      tags={"Document"},
     *      description="Get Comments Of Test",
     *      produces={"application/json"},
     *      @SWG\Parameter(
     *          name="id",
     *          description="id of Test",
    ...
    public function getComments($id)
    {
        ...
        return $this->sendResponse($comments->toArray(), 'Comments Of Test retrieved successfully');
    }

    public function getTests($category_id)
    {
        $category = $this->categoryTestRepository->findByField('id', '=', $category_id,['*'],false)->first();
        $tests = $category->test;
        if (empty($tests)) {
            return $this->sendError('Test not found');
        }
        return $this->sendResponse($tests->toArray(), 'Get test successfully');
    }
}
