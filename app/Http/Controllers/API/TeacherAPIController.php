...
class TeacherAPIController extends AppBaseController
{
     ...
     * @SWG\Get(
     *      path="/teachers/free",
     *      summary="Get a listing of the teacherFrees.",
     *      tags={"Teacher"},
     *      description="Get all teacherFree",
     *      produces={"application/json"},
     *      @SWG\Response(
     *          response=200,
     *          description="successful operation",
     ...
     * )
     */
    public function teacherFree(Request $request)
    {
        $this->teacherRepository->pushCriteria(new RequestCriteria($request));
        $this->teacherRepository->pushCriteria(new LimitOffsetCriteria($request));
        $teachers = $this->teacherRepository->findWhere([['center_id','=','0']]);
